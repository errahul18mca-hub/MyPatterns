...
let token;
try {
	token = JSON.parse(fs.readFileSync(atPath));
} catch (err) {
	token = { access_token: 'dummy_token', time: 100 };
...
module.exports.auth = async (username, password, force) => {
	...
	if (!token || Date.now() > token.time || force) {
		...
		try {
			...
			time = Date.now() + res.data.expires_in * 1000;
...
async function axioCall(url, symbol, response, symbolId) {
	...
	try {
		...
		} else if (err.response && err.response.data === 'API request quota exceeded!') {
			// if (data.status.startsWith('QVBJIGN')) data.status = 'Error : Rate limit exceeded';
			console.log('Rate limit reached - Requests are being staggered')
