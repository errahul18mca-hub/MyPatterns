...
const { bidDeactivatedMessage, reconnectionTime, L2Keys } = utils;
...
module.exports.getMarketStatus = () => {
	connection.send(JSON.stringify({ method: 'getmarketstatus' }));
};
...
module.exports.isSocketConnected = () => {
	return connection !== null && !socketCloseStates.includes(connection.readyState);
};

const connect = () => {
	if (!connection || (connection && socketCloseStates.includes(connection.readyState))) {
...
function socketonmessage(e) {
	...
	} else if (jsonObj.greeks != null) {
		const greeksArray = jsonObj.greeks;
		rtFeed.emit('greeks', handleGreeksData(greeksArray));
	} else if (bidaskData && jsonObj.bidask != null) {
		const bidaskArray = jsonObj.bidask;
		rtFeed.emit('bidask', handleBidaskData(bidaskArray));
	} else if (bidaskData && jsonObj.bidaskL2 != null) {
		const bidaskL2Array = jsonObj.bidaskL2;
		rtFeed.emit('bidaskL2', handleBidaskL2Data(bidaskL2Array));
...
function handleRealTimeData(tradeArray) {
	...
	return {
		...
		OI,
		Price_Change: +(+tradeArray[2] - +tradeArray[9]).toFixed(2),
		Price_Change_Percentage: +(((+tradeArray[2] - +tradeArray[9]) / +tradeArray[9]) * 100).toFixed(2),
		OI_Change: OI ? +(+tradeArray[10] - +tradeArray[11]).toFixed(2) : 0,
		OI_Change_Percentage: OI ? +(((+tradeArray[10] - +tradeArray[11]) / +tradeArray[11]) * 100).toFixed(2) : 0,
...
function handleGreeksData(greeksArray) {
	return {
		Symbol: touchlineMap[greeksArray[0]],
		SymbolId: greeksArray[0],
		Time: greeksArray[1],
		IV: +greeksArray[2],
		Delta: +greeksArray[3],
		Theta: +greeksArray[4],
...
function handleBidaskL2Data(bidaskL2Array) {
	const bidAskL2 = { Symbol: touchlineMap[bidaskL2Array[0]], SymbolId: +bidaskL2Array[0], Time: bidaskL2Array[1] };
	for (let i = 2; i < 35; i++) {
		bidAskL2[L2Keys[i]] = +bidaskL2Array[i];
	}
...
function creatUrl(value) {
	return `wss://${value}.truedata.in`;
}
